...
export default function Sleep(props) {
    ...
    const [slept ,setSleep] = useState(3.25)
    const [total ,setTotal] = useState(8.30)
    const [started , setStarted] = useState(false)
    const [seconds , setSeconds] = useState(0)

    const onStart = ()=> setInterval(() => { setSeconds(seconds + 1)},1000)
    ...
    >
      ...
      <View style={styles.container}>
          ...
          >
            ...
            <View>
                <Text style={{fontSize:60,fontWeight:'bold',alignSelf:'center',color:"#1C2023"}}>Sleep</Text>
            </View>
                    ...
                    <AppButton onPress={()=>setStarted(!started)} text={started?'Stop Sleep Timer':'Start Sleep Timer'} color={1}/>
                     {/* <Text>{seconds}</Text>            */}
                     </View>
      </View>

import React from "react";
import { View, Text } from "react-native";
const data = [
  {
    Name: "lamb leg",
    Kind: "meat",
    KCal: 203,
    Gram: 100,
    Protein: 27.61,
    Fats: 9.7,
    Description:
      "Lamb Leg source of many vitamins and minerals, including iron, zinc, and vitamin B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb loin",
    Kind: "meat",
    KCal: 313,
    Gram: 100,
    Protein: 24.53,
    Fats: 23.09,
    Description: "Low in calories and encourages the burning of fat",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb flank",
    Kind: "meat",
    KCal: 364,
    Gram: 100,
    Protein: 14.5,
    Fats: 34.5,
    Description:
      "Lamb Flanks can help reduce levels of bad cholesterol in your blood, ",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb shoulder",
    Kind: "meat",
    KCal: 275,
    Gram: 100,
    Protein: 22.42,
    Fats: 19.89,
    Description:
      "Lamb Shoulder source of many vitamins and minerals, including iron, zinc, and vitamin B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb head",
    Kind: "meat",
    KCal: 282,
    Gram: 100,
    Protein: 19.4,
    Fats: 23,
    Description:
      "Lamb Head source of many vitamins and minerals, including iron, zinc, and vitamin B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb rib",
    Kind: "meat",
    KCal: 359,
    Gram: 100,
    Protein: 22.04,
    Fats: 29.47,
    Description:
      "Lamb Rib is an outstanding source of many vitamins and minerals, including iron, zinc, and vitamin B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb neck",
    Kind: "meat",
    KCal: 235,
    Gram: 100,
    Protein: 18.4,
    Fats: 17.6,
    Description:
      "Lamb Neck can help reduce levels of bad cholesterol in your blood, ",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb breast",
    Kind: "meat",
    KCal: 230,
    Gram: 100,
    Protein: 24,
    Fats: 15,
    Description:
      "Regular consumption of Lamb Breast may promote muscle growth, maintenance, and performance. In addition, it helps prevent anemia.",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lamb shank",
    Kind: "meat",
    KCal: 207,
    Gram: 100,
    Protein: 24,
    Fats: 11,
    Description:
      "Lamb Shank is a good source of iron, magnesium, selenium, and omega-3 fatty acids",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey breast",
    Kind: "meat",
    KCal: 135,
    Gram: 100,
    Protein: 30.06,
    Fats: 0.74,
    Description:
      "The tryptophan content in turkey may help to support healthy levels of serotonin in the body",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey drumstick",
    Kind: "meat",
    KCal: 206,
    Gram: 100,
    Protein: 27.64,
    Fats: 9.7,
    Description:
      "Turkey meat is a particularly rich source of B vitamins, including B3, B6",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey thigh",
    Kind: "meat",
    KCal: 206,
    Gram: 100,
    Protein: 27.64,
    Fats: 9.7,
    Description:
      "Turkey Thigh is a particularly rich source of B vitamins, B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey wing",
    Kind: "meat",
    KCal: 148,
    Gram: 100,
    Protein: 29.09,
    Fats: 3.22,
    Description:
      "Turkey Wing is a very rich source of Protein, niacin, vitamin B6 and the amino acid tryptothan",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey drummette",
    Kind: "meat",
    KCal: 380,
    Gram: 100,
    Protein: 36,
    Fats: 25,
    Description:
      "Turkey Drummette is a very rich source of Protein, niacin, vitamin B6 and the amino acid tryptothan",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey giblets",
    Kind: "meat",
    KCal: 105,
    Gram: 100,
    Protein: 15,
    Fats: 4,
    Description:
      "Turkey Giblets contains several vitamins including Vitamin B12, which is essential for brain function and forming white blood cells",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "turkey Tenderloin",
    Kind: "meat",
    KCal: 120,
    Gram: 100,
    Protein: 28,
    Fats: 0,
    Description:
      "Turkey Tenderloin is a particularly rich source of B vitamins",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "brisket flat cut",
    Kind: "meat",
    KCal: 342,
    Gram: 100,
    Protein: 24.84,
    Fats: 26.15,
    Description: "a lean cut of meat, making it a healthier red meat option",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless short ribs",
    Kind: "meat",
    KCal: 471,
    Gram: 100,
    Protein: 22,
    Fats: 42,
    Description:
      "good source of Protein and a number of essential vitamins and minerals",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shoulder petite tender medallions",
    Kind: "meat",
    KCal: 122,
    Gram: 100,
    Protein: 17.47,
    Fats: 5.29,
    Description: "best cooked quickly over high heat",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shoulder petite tender",
    Kind: "meat",
    KCal: 176,
    Gram: 100,
    Protein: 26,
    Fats: 7,
    Description:
      " the most tender beef muscles and is said to be white-tablecloth quality ",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless shoulder pot roast",
    Kind: "meat",
    KCal: 220,
    Gram: 100,
    Protein: 33.3,
    Fats: 8.3,
    Description: "Pot roast doesnt typically get a nutritional nod",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless shoulder steak",
    Kind: "meat",
    KCal: 125,
    Gram: 100,
    Protein: 21.4,
    Fats: 4.5,
    Description: "Always go for cuts of beef that are over 93 percent lean",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shoulder center ranch steak",
    Kind: "meat",
    KCal: 180,
    Gram: 100,
    Protein: 25,
    Fats: 8.1,
    Description: "Good for grilling or broiling",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless chuck eye steak",
    Kind: "meat",
    KCal: 276,
    Gram: 100,
    Protein: 25,
    Fats: 20,
    Description: "good source of Vitamin B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shoulder top blade steak",
    Kind: "meat",
    KCal: 176,
    Gram: 100,
    Protein: 18.99,
    Fats: 10.52,
    Description: "good source of vitamin B6",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shoulder top blade steak flat iron",
    Kind: "meat",
    KCal: 139,
    Gram: 100,
    Protein: 20.4,
    Fats: 6.4,
    Description: "a good alternative to more expensive steaks",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless chuck steak",
    Kind: "meat",
    KCal: 191,
    Gram: 100,
    Protein: 32,
    Fats: 6.8,
    Description:
      "help with nervous system function and red blood cell production",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless chuck pot roast",
    Kind: "meat",
    KCal: 276.5,
    Gram: 100,
    Protein: 25,
    Fats: 20,
    Description: "High in Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "flank steak",
    Kind: "meat",
    KCal: 192,
    Gram: 100,
    Protein: 28,
    Fats: 8,
    Description: "fewer calories and more Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shank cross-cut",
    Kind: "meat",
    KCal: 201,
    Gram: 100,
    Protein: 34.1,
    Fats: 6.4,
    Description: "fewer calories and more Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless ribeye steak",
    Kind: "meat",
    KCal: 354,
    Gram: 100,
    Protein: 23,
    Fats: 29,
    Description: "Ribeye is loaded with muscle-building Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "back ribs",
    Kind: "meat",
    KCal: 386,
    Gram: 100,
    Protein: 14,
    Fats: 34,
    Description: "lots of Fats !",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless ribeye roast",
    Kind: "meat",
    KCal: 266,
    Gram: 100,
    Protein: 25.91,
    Fats: 17.32,
    Description: "blood-aiding iron, energy-maintaining vitamin B12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "rib steak",
    Kind: "meat",
    KCal: 271,
    Gram: 100,
    Protein: 25,
    Fats: 19,
    Description: "good handful of other vitamins and minerals",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "rib roast",
    Kind: "meat",
    KCal: 283.5,
    Gram: 100,
    Protein: 21,
    Fats: 35,
    Description: "ribeye steaks are durable steaks",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "round steak",
    Kind: "meat",
    KCal: 181.6,
    Gram: 100,
    Protein: 28,
    Fats: 7,
    Description: "Beef Provides a Large Source of L-Carnitine",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "round roast",
    Kind: "meat",
    KCal: 166,
    Gram: 100,
    Protein: 21.49,
    Fats: 8.24,
    Description: "good source of vitamin B12 and B6",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "round tip roast",
    Kind: "meat",
    KCal: 119,
    Gram: 100,
    Protein: 21.11,
    Fats: 3.2,
    Description: "Its one of the best cuts for roast beef",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "bottom round roast",
    Kind: "meat",
    KCal: 181,
    Gram: 100,
    Protein: 28,
    Fats: 7,
    Description: "youre looking for a rich Protein source? here",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: " steak western griller",
    Kind: "meat",
    KCal: 252,
    Gram: 100,
    Protein: 27.29,
    Fats: 15.01,
    Description:
      "Grilled meat is lower in fat and calories compared with others",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "tip steak",
    Kind: "meat",
    KCal: 164,
    Gram: 100,
    Protein: 20.64,
    Fats: 8.55,
    Description: "Beef is Extremely Rich in Minerals",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sirloin tip center roast",
    Kind: "meat",
    KCal: 210.6,
    Gram: 100,
    Protein: 25.9,
    Fats: 11.1,
    Description: " it has tough muscle fibers inside",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sirloin tip center steak",
    Kind: "meat",
    KCal: 201,
    Gram: 100,
    Protein: 20.3,
    Fats: 12.71,
    Description: "Beef has a lot of collagen in it",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sirloin tip side steak",
    Kind: "meat",
    KCal: 183,
    Gram: 100,
    Protein: 30,
    Fats: 5,
    Description:
      "that same cut of meat can be distressingly tough, chewy or dry",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "skirt steak",
    Kind: "meat",
    KCal: 220,
    Gram: 100,
    Protein: 26,
    Fats: 12,
    Description:
      "Skirt Steak is an excellent source of Protein, Riboflavin, Niacin, Vitamin B6, Vitamin B12, Zinc, and Selenium",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "top loin steak boneless",
    Kind: "meat",
    KCal: 201,
    Gram: 100,
    Protein: 20.3,
    Fats: 12.71,
    Description:
      "Top Loin Steak Boneless cut is jam-packed with around 50 Grams of Protein which is slightly more than most other lean cuts of steak",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "tenderloin roast",
    Kind: "meat",
    KCal: 324,
    Gram: 100,
    Protein: 24,
    Fats: 25,
    Description:
      "tenderloin roast contains L-carnitine which is an amino acid that occurs naturally in meat products",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "T-bone steak",
    Kind: "meat",
    KCal: 246.9,
    Gram: 100,
    Protein: 24,
    Fats: 16,
    Description:
      "T-bone Steak is one of the best Protein rich foods. Steak is one of the best Protein sources, and Protein is important for pretty much every cell in your body",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "boneless top sirloin steak",
    Kind: "meat",
    KCal: 250.5,
    Gram: 100,
    Protein: 26,
    Fats: 15,
    Description:
      "Boneless Top Sirloin steak tends to be one of the leanest cuts of beef making it a great option in a health-conscious die",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "chicken breast",
    Kind: "meat",
    KCal: 165,
    Gram: 100,
    Protein: 31,
    Fats: 3.6,
    Description: "an excellent source of lean Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "chicken wings",
    Kind: "meat",
    KCal: 203,
    Gram: 100,
    Protein: 30.5,
    Fats: 8.1,
    Description:
      "help with weight management and reduce the risk of heart disease",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "chicken drumsticks / legs",
    Kind: "meat",
    KCal: 172,
    Gram: 100,
    Protein: 28.3,
    Fats: 5.7,
    Description: " they're high in collagen",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "chicken thighs",
    Kind: "meat",
    KCal: 209,
    Gram: 100,
    Protein: 24,
    Fats: 8,
    Description:
      "contain riboflavin, vitamins B6 and B12, iron, and zinc of vitamins",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "whole chicken",
    Kind: "meat",
    KCal: 165,
    Gram: 100,
    Protein: 31,
    Fats: 3.6,
    Description: " a good source of Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "orange",
    Kind: "fruits",
    KCal: 48.1,
    Gram: 100,
    Protein: 0.9,
    Fats: 0.9,
    Description: "Protects your cells from damage",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "grapefruit",
    Kind: "fruits",
    KCal: 42.1,
    Gram: 100,
    Protein: 0.8,
    Fats: 0.1,
    Description: " theyre a good source of vitamin A ",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "mandarin",
    Kind: "fruits",
    KCal: 53.3,
    Gram: 100,
    Protein: 0.8,
    Fats: 0,
    Description:
      "The body turns beta-carotene and beta-cryptoxanthin into vitamin A ",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lemon",
    Kind: "fruits",
    KCal: 27.9,
    Gram: 100,
    Protein: 1.1,
    Fats: 0.3,
    Description: "Lemons contain a high amount of vitamin C",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lime",
    Kind: "fruits",
    KCal: 30.1,
    Gram: 100,
    Protein: 0.8,
    Fats: 0.2,
    Description: "Limes are a good source of magnesium and potassium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "nectarine",
    Kind: "fruits",
    KCal: 44.2,
    Gram: 100,
    Protein: 1.1,
    Fats: 0.3,
    Description:
      "Nectarine support iron absorption, boost weight loss, and enhance skin health",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "apricot",
    Kind: "fruits",
    KCal: 48,
    Gram: 100,
    Protein: 1.4,
    Fats: 0.4,
    Description:
      "Apricot rich in vitamin A, beta-carotene , excellent for promoting eye health",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "peach",
    Kind: "fruits",
    KCal: 39,
    Gram: 100,
    Protein: 0.91,
    Fats: 0,
    Description:
      "Peach is a great source of fiber, potassium and vitamins A and C",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "plum",
    Kind: "fruits",
    KCal: 45.9,
    Gram: 100,
    Protein: 0.7,
    Fats: 0.3,
    Description:
      "Plums contain antioxidant in huge amounts that may help prevent age-related macular degeneration",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "banana",
    Kind: "fruits",
    KCal: 88.7,
    Gram: 100,
    Protein: 1.1,
    Fats: 0.3,
    Description: "rich in potassium, fiber and natural sugars",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "mango",
    Kind: "fruits",
    KCal: 59.8,
    Gram: 100,
    Protein: 0.8,
    Fats: 0.4,
    Description: "good source of fiber and antioxidants, including vitamin C",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "strawberry",
    Kind: "fruits",
    KCal: 32.5,
    Gram: 100,
    Protein: 0.7,
    Fats: 0.3,
    Description: "high levels of antioxidants known as polyphenols",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "raspberry",
    Kind: "fruits",
    KCal: 52.6,
    Gram: 100,
    Protein: 1.2,
    Fats: 0.7,
    Description:
      "essential to heart function, and proven to lower blood pressure",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "kiwi",
    Kind: "fruits",
    KCal: 60.9,
    Gram: 100,
    Protein: 1.1,
    Fats: 0.5,
    Description: "high in Vitamin C and dietary fiber",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "passionfruit",
    Kind: "fruits",
    KCal: 97,
    Gram: 100,
    Protein: 2.2,
    Fats: 0.7,
    Description: "protects your cells from damage",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "watermelon",
    Kind: "fruits",
    KCal: 30.4,
    Gram: 100,
    Protein: 0.6,
    Fats: 0.2,
    Description:
      "Your heart enjoys the perks of all the lycopene watermelon contains",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "rockmelon",
    Kind: "fruits",
    KCal: 33.7,
    Gram: 100,
    Protein: 0.8,
    Fats: 0.2,
    Description:
      "Rockmelon is a powerful antioxidant that protects your cells from damage, Full of vitamin A",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "honeydew melon",
    Kind: "fruits",
    KCal: 35.9,
    Gram: 100,
    Protein: 0.5,
    Fats: 0.1,
    Description:
      "Honeydew Melon is low in fat and contains several key vitamins and minerals",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "Tomato",
    Kind: "fruits",
    KCal: 18,
    Gram: 100,
    Protein: 0.9,
    Fats: 0.04,
    Description: "Tomatoes have potassium, vitamins B and E",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "avocado",
    Kind: "fruits",
    KCal: 160.1,
    Gram: 100,
    Protein: 2,
    Fats: 15,
    Description:
      "Avocado are an excellent source of potassium, folate and fibre",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "apple",
    Kind: "fruits",
    KCal: 52.1,
    Gram: 100,
    Protein: 0.3,
    Fats: 0.2,
    Description: "Apples Can Support a Healthy Immune System",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "pear",
    Kind: "fruits",
    KCal: 57.1,
    Gram: 100,
    Protein: 0.39,
    Fats: 0.1,
    Description:
      "Pears are rich in essential antioxidants, plant compounds, and dietary fiber",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "Pomegranates",
    Kind: "fruits",
    KCal: 83,
    Gram: 100,
    Protein: 1.7,
    Fats: 0.1,
    Description:
      "Pomegranates good in improving gut health, digestion, and keep bowel diseases at bay",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lettuce",
    Kind: "vegatables",
    KCal: 14,
    Gram: 100,
    Protein: 0.9,
    Fats: 1.3,
    Description: "source of vitamin K, which helps strengthen bones",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "spinach",
    Kind: "vegatables",
    KCal: 23,
    Gram: 100,
    Protein: 2.9,
    Fats: 0.4,
    Description: "High amounts of oxalic acid in spinach",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "Swiss chard",
    Kind: "vegatables",
    KCal: 14,
    Gram: 100,
    Protein: 0.9,
    Fats: 0.1,
    Description: "excellent source of vitamins K, A, and C",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cabbage",
    Kind: "vegatables",
    KCal: 23,
    Gram: 100,
    Protein: 1.3,
    Fats: 0.1,
    Description: "heart-protective antioxidants",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cauliflower",
    Kind: "vegatables",
    KCal: 25,
    Gram: 100,
    Protein: 2,
    Fats: 0,
    Description: "Rich in sulforaphane",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "Brussels sprouts",
    Kind: "vegatables",
    KCal: 36,
    Gram: 100,
    Protein: 2.6,
    Fats: 0.1,
    Description: "rotect against cancers of the stomach",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "broccoli",
    Kind: "vegatables",
    KCal: 35,
    Gram: 100,
    Protein: 2.4,
    Fats: 0.4,
    Description: "Packed With Vitamins, Minerals and Bioactive Compounds",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "pumpkin",
    Kind: "vegatables",
    KCal: 20,
    Gram: 100,
    Protein: 0.7,
    Fats: 0.1,
    Description: "pumpkins offer vitamin C, vitamin E, iron, and folate",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cucumber",
    Kind: "vegatables",
    KCal: 15,
    Gram: 100,
    Protein: 0.7,
    Fats: 0.1,
    Description: "high in many important vitamins and minerals",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "zucchini",
    Kind: "vegatables",
    KCal: 15,
    Gram: 100,
    Protein: 1.1,
    Fats: 0.4,
    Description: "rich in multiple antioxidants",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "potato",
    Kind: "vegatables",
    KCal: 93,
    Gram: 100,
    Protein: 2.5,
    Fats: 0.1,
    Description: "a good source of fiber",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sweet potato",
    Kind: "vegatables",
    KCal: 90,
    Gram: 100,
    Protein: 2,
    Fats: 0.1,
    Description: "good source of most of our B vitamins and vitamin C",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "yam",
    Kind: "vegatables",
    KCal: 118,
    Gram: 100,
    Protein: 1.53,
    Fats: 0.17,
    Description: "may boost brain health",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "celery",
    Kind: "vegatables",
    KCal: 14,
    Gram: 100,
    Protein: 0.69,
    Fats: 0.17,
    Description: "Youll enjoy vitamins A, K, and C, plus minerals",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "asparagus",
    Kind: "vegatables",
    KCal: 20,
    Gram: 100,
    Protein: 2.2,
    Fats: 0.12,
    Description: " potassium, fiber, thiamin, and vitamins A, B6 and more",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "onion",
    Kind: "vegatables",
    KCal: 42,
    Gram: 100,
    Protein: 0.92,
    Fats: 0.08,
    Description: "contain antioxidants and compounds that fight inflammation",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "garlic",
    Kind: "vegatables",
    KCal: 149,
    Gram: 100,
    Protein: 6.36,
    Fats: 0.5,
    Description: "May Help Lower Blood Pressure",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "shallot",
    Kind: "vegatables",
    KCal: 72,
    Gram: 100,
    Protein: 2.5,
    Fats: 0.1,
    Description:
      "may improve blood sugar levels , circulation, seasonal allergies, and heart and bone health",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cider",
    Kind: "drinks",
    KCal: 47,
    Gram: 100,
    Protein: 0.06,
    Fats: 0.11,
    Description:
      "lowering your risk of certain types of cancer, diabetes, and heart disease",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sparkling water",
    Kind: "drinks",
    KCal: 0,
    Gram: 100,
    Protein: 0,
    Fats: 0,
    Description:
      "empties the stomach and could possibly make someone feel hungry",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "soda drinks",
    Kind: "drinks",
    KCal: 38,
    Gram: 100,
    Protein: 0.07,
    Fats: 0.01,
    Description:
      "Soda is not good for a persons health because it contains lots of sugar",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "barley",
    Kind: "drinks",
    KCal: 354,
    Gram: 100,
    Protein: 12,
    Fats: 2.3,
    Description: "Rich in Many Beneficial Nutrients",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "coffee",
    Kind: "drinks",
    KCal: 0,
    Gram: 100,
    Protein: 0.1,
    Fats: 0,
    Description: "Can Improve Energy Levels and Make You Smarter",
    MealTimes: [{ MealId: 0 }, { MealId: 3 }],
  },
  {
    Name: "hot chocolate",
    Kind: "drinks",
    KCal: 77,
    Gram: 100,
    Protein: 3.5,
    Fats: 2.3,
    Description: "lower blood pressure",
    MealTimes: [{ MealId: 0 }, { MealId: 3 }],
  },
  {
    Name: "lemon tea",
    Kind: "drinks",
    KCal: 2,
    Gram: 100,
    Protein: 0.03,
    Fats: 0,
    Description: "Detoxifies The Body",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "juice drinks",
    Kind: "drinks",
    KCal: 55,
    Gram: 100,
    Protein: 0,
    Fats: 0.09,
    Description:
      "They say juicing can reduce your risk of cancer, boost your immune system, remove toxins from your body",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "green tea",
    Kind: "drinks",
    KCal: 1,
    Gram: 100,
    Protein: 0,
    Fats: 0,
    Description:
      "Certain compounds in green tea may inhibit the absorption of iron and other minerals, so its best to drink it between meals",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "orange juice",
    Kind: "drinks",
    KCal: 45,
    Gram: 100,
    Protein: 0.7,
    Fats: 0.2,
    Description:
      "Orange juice is a favorite beverage high in antioxidants and micronutrients like vitamin C, folate, and potassium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "apple juice",
    Kind: "drinks",
    KCal: 47,
    Gram: 100,
    Protein: 0.06,
    Fats: 0.11,
    Description:
      "Apple Juice  is a good source of nutrients like vitamin C and potassium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "coconut water",
    Kind: "drinks",
    KCal: 19,
    Gram: 100,
    Protein: 0.72,
    Fats: 0.2,
    Description:
      "Coconut Water May help prevent kidney stones, May help lower blood sugar for people living with diabetes",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "Salmon",
    Kind: "sea food",
    KCal: 208,
    Gram: 100,
    Protein: 20,
    Fats: 13,
    Description: "a good source of Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cod",
    Kind: "sea food",
    KCal: 82,
    Gram: 100,
    Protein: 18,
    Fats: 0.7,
    Description: "a low-fat source of Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "trout",
    Kind: "sea food",
    KCal: 140,
    Gram: 100,
    Protein: 20,
    Fats: 6,
    Description: "niacin, vitamin B12, and omega 3",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "tuna",
    Kind: "sea food",
    KCal: 129,
    Gram: 100,
    Protein: 29,
    Fats: 0.6,
    Description: "high levels of omega-3 fatty acids",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sardines",
    Kind: "sea food",
    KCal: 208,
    Gram: 100,
    Protein: 25,
    Fats: 11,
    Description: "excellent source of vitamin B-12",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "flounder and sole",
    Kind: "sea food",
    KCal: 91,
    Gram: 100,
    Protein: 18.84,
    Fats: 1.19,
    Description: "plenty of Protein",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "barramundi",
    Kind: "sea food",
    KCal: 108,
    Gram: 100,
    Protein: 22.8,
    Fats: 1.8,
    Description:
      "attributed to many of the heart healthy benefits of this diet",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "pollock",
    Kind: "sea food",
    KCal: 81,
    Gram: 100,
    Protein: 17,
    Fats: 0,
    Description: "good source of lean Protein and low in saturated fat",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "tilapia",
    Kind: "sea food",
    KCal: 128,
    Gram: 100,
    Protein: 26,
    Fats: 2.7,
    Description: "helps your body make DNA",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "haddock",
    Kind: "sea food",
    KCal: 90,
    Gram: 100,
    Protein: 20,
    Fats: 0.6,
    Description: "build bone strength and regulate your heart rate",
    MealTimes: [{ MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "white bread",
    Kind: "bakery",
    KCal: 285,
    Gram: 100,
    Protein: 7.84,
    Fats: 5.73,
    Description: "is fortified with calcium and four medium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "wheat bread",
    Kind: "bakery",
    KCal: 252,
    Gram: 100,
    Protein: 12,
    Fats: 3.5,
    Description: "rich in Protein, fiber, B vitamins",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "whole wheat bread",
    Kind: "bakery",
    KCal: 272,
    Gram: 100,
    Protein: 8.36,
    Fats: 6.17,
    Description: "including Protein, fiber, B vitamins, antioxidants",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "multigrain bread",
    Kind: "bakery",
    KCal: 251,
    Gram: 100,
    Protein: 10,
    Fats: 3.8,
    Description: "help you to feel satiated",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "whole grain bread",
    Kind: "bakery",
    KCal: 264.8,
    Gram: 100,
    Protein: 13,
    Fats: 4.2,
    Description: "help to lower blood pressure",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sprouted grain bread",
    Kind: "bakery",
    KCal: 198,
    Gram: 100,
    Protein: 7.49,
    Fats: 1.27,
    Description: "Its lower in carbs, higher in Protein and fiber",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sourdough bread",
    Kind: "bakery",
    KCal: 289.2,
    Gram: 100,
    Protein: 12,
    Fats: 1.8,
    Description: "easy to digest and Natural origins",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "rye bread",
    Kind: "bakery",
    KCal: 259.1,
    Gram: 100,
    Protein: 9,
    Fats: 3.3,
    Description: "especially B vitamins",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "rumpermickel bread",
    Kind: "bakery",
    KCal: 250,
    Gram: 100,
    Protein: 9,
    Fats: 3.1,
    Description: "Helps reduce blood sugar levels",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "brioche bread",
    Kind: "bakery",
    KCal: 346,
    Gram: 100,
    Protein: 9.74,
    Fats: 13.18,
    Description:
      "high fat and Protein contents of these ingredients is what makes the bread so special",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "challah bread",
    Kind: "bakery",
    KCal: 283,
    Gram: 100,
    Protein: 9.5,
    Fats: 6,
    Description: "it has a rich flavor",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "flatbread bread",
    Kind: "bakery",
    KCal: 311,
    Gram: 100,
    Protein: 8.49,
    Fats: 11.59,
    Description: "High Protein & Fiber",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "flaxseeds",
    Kind: "seeds",
    KCal: 534,
    Gram: 100,
    Protein: 18.29,
    Fats: 42.16,
    Description: "Flaxseeds is loaded with fiber, Protein and potassium",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "chia seeds",
    Kind: "seeds",
    KCal: 490,
    Gram: 100,
    Protein: 15.62,
    Fats: 30.75,
    Description:
      "Chia seeds are an excellent plant-based source of omega-3 Fats",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "pumpkin seeds",
    Kind: "seeds",
    KCal: 446,
    Gram: 100,
    Protein: 18,
    Fats: 19,
    Description:
      "Pumpkin seeds are a great source for lots of minerals, including zinc",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sunflower seeds",
    Kind: "seeds",
    KCal: 570,
    Gram: 100,
    Protein: 22.78,
    Fats: 49.57,
    Description:
      "Sunflower seeds have a good amount of minerals, B vitamins and antioxidants like vitamin E and selenium",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "hemp seeds",
    Kind: "seeds",
    KCal: 566,
    Gram: 100,
    Protein: 38.03,
    Fats: 45.5,
    Description: "Hemp seeds are rich in vitamin E and potassium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "sesame seeds",
    Kind: "seeds",
    KCal: 567,
    Gram: 100,
    Protein: 16.96,
    Fats: 48,
    Description:
      "In addition to minerals and fiber, sesame seeds are high in selenium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "wild rice",
    Kind: "seeds",
    KCal: 101,
    Gram: 100,
    Protein: 3.99,
    Fats: 0.34,
    Description: "Wild rice is a good source of fiber",
    MealTimes: [{ MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "quinoa",
    Kind: "seeds",
    KCal: 143,
    Gram: 100,
    Protein: 5.01,
    Fats: 2.22,
    Description:
      "Along with amino acids, and vitamin E. Quinoa also contains an antioxidant called quercetin",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "poppy seeds",
    Kind: "seeds",
    KCal: 533,
    Gram: 100,
    Protein: 18.04,
    Fats: 44.7,
    Description:
      "Just one teaspoon of tiny poppy seeds contains up to four percent of your recommended daily intake of phosphorous, calcium, and iron",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "pine nuts",
    Kind: "seeds",
    KCal: 673,
    Gram: 100,
    Protein: 13.69,
    Fats: 68.37,
    Description:
      "Pine nuts contain all of the amino acids along with: vitamin A, thiamin, riboflavin, niacin, vitamin E, copper",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "peanut",
    Kind: "seeds",
    KCal: 567,
    Gram: 100,
    Protein: 25.8,
    Fats: 49.24,
    Description:
      "Peanuts help prevent heart disease by lowering cholesterol levels",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "beans",
    Kind: "seeds",
    KCal: 151,
    Gram: 100,
    Protein: 5.54,
    Fats: 5.15,
    Description:
      "Beans help release short-chain fatty acids (SCFA) that strengthen your intestine cells",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "roasted cashew",
    Kind: "seeds",
    KCal: 553,
    Gram: 100,
    Protein: 18.22,
    Fats: 43.85,
    Description: "Cashews are rich in fiber, Protein, and healthy Fats",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "roasted pistachios",
    Kind: "seeds",
    KCal: 570,
    Gram: 100,
    Protein: 21.05,
    Fats: 45.82,
    Description:
      "Contains a high levels of unsaturated fatty acids and potassium",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "chickpea seeds",
    Kind: "seeds",
    KCal: 180,
    Gram: 100,
    Protein: 9.54,
    Fats: 2.99,
    Description:
      " Chickpeas are high in dietary fiber, especially a soluble fiber called raffinose",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "lupine",
    Kind: "seeds",
    KCal: 371,
    Gram: 100,
    Protein: 36.17,
    Fats: 9.74,
    Description:
      " Lupin has been shown to lower blood pressure and insulin sensitivity",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cowpea",
    Kind: "seeds",
    KCal: 44,
    Gram: 100,
    Protein: 3.3,
    Fats: 0.3,
    Description: "Cowpea regulates normal functioning of the cardiac muscles",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "peas",
    Kind: "seeds",
    KCal: 128,
    Gram: 100,
    Protein: 8.8,
    Fats: 0.68,
    Description:
      "Peas are a good source of vitamins C and E, zinc, and other antioxidants",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "yellow lentils",
    Kind: "seeds",
    KCal: 332,
    Gram: 100,
    Protein: 25.4,
    Fats: 2.4,
    Description:
      "Yellow Lentils are rich in fibre, folate and potassium making them a great choice for the heart and for managing blood pressure",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "red lentils",
    Kind: "seeds",
    KCal: 143,
    Gram: 100,
    Protein: 12.1,
    Fats: 1,
    Description: "They are high in vitamin B6, pantothenic acid and thiamine",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "milk",
    Kind: "dairy",
    KCal: 42,
    Gram: 100,
    Protein: 3.4,
    Fats: 1,
    Description: "calcium, phosphorus, B vitamins, potassium and vitamin D",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "butter",
    Kind: "dairy",
    KCal: 716,
    Gram: 100,
    Protein: 0.09,
    Fats: 81,
    Description: "bone growth and development",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cheese",
    Kind: "dairy",
    KCal: 402,
    Gram: 100,
    Protein: 25,
    Fats: 33,
    Description: "great source of calcium, fat, and Protein",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "yogurt",
    Kind: "dairy",
    KCal: 58,
    Gram: 100,
    Protein: 10,
    Fats: 0.4,
    Description: "Its High in Protein",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "cream",
    Kind: "dairy",
    KCal: 195,
    Gram: 100,
    Protein: 2.7,
    Fats: 19,
    Description: "including vitamin A, D, E, and K",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "ice cream",
    Kind: "dairy",
    KCal: 207,
    Gram: 100,
    Protein: 3.5,
    Fats: 11,
    Description: "its composition vitamins A, B, C and D",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "whey",
    Kind: "dairy",
    KCal: 26,
    Gram: 100,
    Protein: 0.9,
    Fats: 0.4,
    Description: "gain muscle mass and strength",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "casein",
    Kind: "dairy",
    KCal: 365,
    Gram: 100,
    Protein: 77,
    Fats: 1.9,
    Description: "boost muscle growth and aid recovery after exercise",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
  {
    Name: "egg",
    Kind: "dairy",
    KCal: 155.1,
    Gram: 100,
    Protein: 13,
    Fats: 11,
    Description: "rich in the antioxidants lutein and zeaxanthin",
    MealTimes: [{ MealId: 0 }, { MealId: 1 }, { MealId: 2 }, { MealId: 3 }],
  },
];

const JustForAddIdPhoto = () => {
  const func = () => {
    let x = [];
    data.map((e, index) => {
      x.push({ ...e, photoId: index + 2 });
    });
  };
  func();
  return <View></View>;
};

export default JustForAddIdPhoto;
